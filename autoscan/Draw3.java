    ...
    public static void main(String[] args)
        {
            ...
            String keywords = "%mem=32GB\n%nprocshared=32\n#p geom=connect b3lyp/6-31g(d) empiricaldispersion=gd3bj scrf=(pcm,solvent=water) pop=none opt=modredundant";
            String tail = String.format("B %d %d F\nB %d %d F\n\n", fromAtomNumber1, toAtomNumber1, fromAtomNumber2, toAtomNumber2);
            
            for (double distance1 = 1.40; distance1 <= 2.4; distance1 += 0.05)
                {
                    for ( double distance2 = 1.00; distance2 <= 1.90; distance2 += 0.05)
                        {
                            ...
                            String filename = String.format("output3/formamide_4H2O_breakdown2_scan_%3.0f_%3.0f.gjf", distance1 * 100, distance2 * 100);
